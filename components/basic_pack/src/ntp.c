...
#define NTP_SERVER_IP           "ntp.ntsc.ac.cn"     /*国家授时中心 IP*/
...
int get_ntp_time(int sk, struct addrinfo *addr, struct ntp_packet *ret_time) {
    ...
    /*客户端给服务器端发送NTP协议数据包*/
    result = sendto(sk, data, packet_len, 0, addr->ai_addr, data_len);
    if (result < 0) {
        perror("sendto");
        return 0;
    }
    ...
    if (select(sk + 1, &pending_data, NULL, NULL, &block_time) > 0) {
        ...
        if (strncmp(protocol, TIME, strlen(TIME)) == 0) {
...
int ntp_update(void)
{
    ...
    LOGI("start sync time with %s", NTP_SERVER_IP);
